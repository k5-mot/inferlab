...
async def run_dataflow(task: dict):
    ...
    for ck in chunks:
        ...
        if not ck.get("id"):
            identity_text = chunk_content or json.dumps(ck, ensure_ascii=False, sort_keys=True)
            ck["id"] = xxhash.xxh64((str(identity_text) + str(ck["doc_id"])).encode("utf-8")).hexdigest()
        if not ck.get("content_with_weight"):
            ck["content_with_weight"] = chunk_content
        if "questions" in ck:
