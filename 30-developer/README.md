...
取得済み資材は `30-developer/registry/{rpm,deb,wheel,npm-packages,docker-archive}` に配置する。`docker compose --env-file .env --profile developer up -d` を実行すると、rpm、deb、wheel、npm package は各 registry へ自動的に反映される。
...
Harbor は公式 installer の `harbor.yml` から生成した `docker-compose.yml` を、起動時だけ root compose に重ねて使う。Docker archive の自動 push は Harbor と同じ Compose project 上で動くため、`30-developer/harbor/prepare-harbor-compose.sh` で生成した後に `30-developer/harbor/harbor-compose.sh up -d` で起動する。
