...
取得済み資材は `30-developer/registry/{pypi,npm,rpm,deb,docker,huggingface,vsix}` に配置する。`30-developer/registry/{pypi,npm,rpm,deb}` に資材を置いて `docker compose --env-file .env --profile developer up -d` を実行すると、各 package registry が自動的に資材を取り込む。
