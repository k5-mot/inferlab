...
BookStackのLinuxServer imageは`/custom-cont-init.d`を実行する。Composeでは、先に`bookstack-custom-init`がrepository内の`custom-cont-init.d`を名前付きvolumeへコピーし、`bookstack`がそのvolumeを読み込む。
