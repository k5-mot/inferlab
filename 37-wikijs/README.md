...
- Wiki.jsを`http://${PUBLIC_HOST}:33700`で表示できる。
...
ブラウザで`http://${PUBLIC_HOST}:33700`を開き、管理者mail address、password、site URLを設定する。site URLには外部から利用する`http://${PUBLIC_HOST}:33700`を指定する。
