...
- `33-registry` は廃止し、開発者向けサービスとして `12-registry` に統合する。
...
`12-registry/assets/` を管理者の作業用資材置場にする。
...
管理者は `12-registry/assets/` に置いた package file、container image tar、model cache 用の情報を、plugin ごとの upload API、push、または cache 作成操作で Pulp repository へ登録する。

運用手順は Git 管理外の `12-registry/MANUAL.md` に分離する。公開管理する要件文書には、具体的な作業環境や作業経路を記載しない。
...
- root の `docker-compose.yml` に `./12-registry/docker-compose.yml` を include する。
...
- package file を `12-registry/assets/` へ置いた場合の plugin 別 upload 手順。
