...
```bash
# 資材置場を作成する。
mkdir -p 12-registry/assets/{pypi,npm,docker,rpm,deb,huggingface}

# registry profileのserviceを起動する。
docker compose --env-file .env --profile registry up -d
...
```bash
# Pulpサーバ側の資材置場を作成する。
mkdir -p 12-registry/assets

# 取得済みassetsをPulpサーバ側の資材置場へ同期する。
rsync -a <ASSETS_SOURCE>/ 12-registry/assets/
```

期待結果: `12-registry/assets/` 配下に `pypi`、`npm`、`docker`、`rpm`、`deb`、`huggingface` がある。
...
for file in 12-registry/assets/pypi/*; do docker compose run --rm --no-deps -v "$PWD/12-registry/assets:/work/assets:ro" pulp-api pulp --base-url http://pulp-web:8080 --username admin --password '<PULP_PASSWORD>' python content create --repository python-internal --relative-path "$(basename "$file")" --file "/work/assets/pypi/$(basename "$file")"; done
```
...
for file in 12-registry/assets/npm/*.tgz; do docker compose run --rm --no-deps -v "$PWD/12-registry/assets:/work/assets:ro" pulp-api pulp --base-url http://pulp-web:8080 --username admin --password '<PULP_PASSWORD>' npm content upload --repository npm-internal --file "/work/assets/npm/$(basename "$file")"; done
```
...
podman load -i 12-registry/assets/docker/hello-world_latest.tar

# ollama/ollamaのtarをlocal image storeへ読み込む。
podman load -i 12-registry/assets/docker/ollama_ollama_latest.tar
...
docker compose run --rm --no-deps -v "$PWD/12-registry/assets:/work/assets:ro" pulp-api pulp --base-url http://pulp-web:8080 --username admin --password '<PULP_PASSWORD>' rpm content upload --repository rpm-internal --directory /work/assets/rpm --publish
```
...
for file in 12-registry/assets/deb/*.deb; do docker compose run --rm --no-deps -v "$PWD/12-registry/assets:/work/assets:ro" pulp-api pulp --base-url http://pulp-web:8080 --username admin --password '<PULP_PASSWORD>' deb content upload --repository deb-internal --file "/work/assets/deb/$(basename "$file")" --distribution stable --component main; done
