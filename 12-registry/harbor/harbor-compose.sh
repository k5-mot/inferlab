...
HARBOR_COMPOSE_FILE="${HARBOR_COMPOSE_FILE:-12-registry/harbor/generated/docker-compose.yml}"
HARBOR_OVERRIDE_FILE="${HARBOR_OVERRIDE_FILE:-12-registry/harbor/docker-compose.override.yml}"

if [ ! -f "$HARBOR_COMPOSE_FILE" ]; then
  echo "ERROR: $HARBOR_COMPOSE_FILE が見つかりません。" >&2
  echo "ERROR: 先に 12-registry/harbor/prepare-harbor-compose.sh を実行してください。" >&2
  exit 1
fi
