...
IMAGE_DIRECTORY="${IMAGE_DIRECTORY:-/srv/docker}"
...
print_usage() {
  cat <<'USAGE'
Usage:
  scripts/install-images.sh [-d|--image-directory DIR] [-h|--help]

Description:
  /srv/docker/*.tar をPodmanまたはDockerへ読み込みます。
...
# image directory配下のcontainer image archiveをengineへloadする。
# 引数:
